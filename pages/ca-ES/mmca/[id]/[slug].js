      ...
      {props.camarkers.length >= 1 ? (
        <div>
          ...
          <p className="align-center">
            <small>
              <Link
                href={`/ca-ES/m-o-g-m-ca/${props.camarkers[0].marca.term_id}/${props.camarkers[0].marca.slug}`}
              >
                <a>veure llistat</a>
              </Link>
            </small>
            ...
            {props.camarkers.length >= 1
              ? props.camarkers.map((marker, index) => (
                  <MarkerComponent
                    ...
                    text={
                      <a href={`/ca-ES/ogmca/${marker.ID}`} title={marker.name}>
                        <span>
                          <img
                            src={
...
export async function getStaticPaths() {
  ...
  const paths = marques.map(m => `/ca-ES/mmca/${m.id}/${m.slug}`);
...
export async function getStaticProps({ params }) {
  ...
  if (!camarkers.data) {
    return { props: {camarkers }, revalidate: 1 };
  } else {
    return { props: { camarkers: '404' } };
  }
}
