    ...
    <Layout ruta={props.ruta}>
      ...
      <nav aria-label="Estás aquí:" role="navigation">
        <ul className="breadcrumbs">
          ...
          <li>
            <Link
              href={`/mmca/${props.ofertagranmarca.marca}/${props.ofertagranmarca._embedded['wp:term'][3][0].slug}`}
            >
      ...
      <section>
        <div className="file">
          ...
          <p className="category">
            <Link
              href={`/mmca/${props.ofertagranmarca.categoria_del_beneficio}/${props.ofertagranmarca._embedded['wp:term'][3][0].slug}`}
            >
              <a
                title={
...
export async function getStaticPaths() {
  ...
  const paths = ofertes.map(o => `/ogmca/${o.id}`);
...
export async function getStaticProps({ params }) {
  const res = await fetch(
    `https://gestorbeneficis.fanoc.org/wp-json/wp/v2/of_gr_m_ca/${params.id}?_embed`
  );
