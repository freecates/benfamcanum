      ...
      {props.camarkers.length >= 1 ? (
        <div>
          ...
          <p className="align-center">
            <small>
              <Link
                href={`/m-o-g-m-ca/${props.camarkers[0].marca.term_id}/${props.camarkers[0].marca.slug}`}
              >
                <a>ver listado</a>
              </Link>
            </small>
            ...
            {props.camarkers.length >= 1
              ? props.camarkers.map((marker, index) => (
                  <MarkerComponent
                    ...
                    text={
                      <a href={`/ogmca/${marker.ID}`} title={marker.name}>
                        <span>
                          <img
                            src={
...
export async function getStaticPaths() {
  ...
  const paths = marques.map(m => `/mmca/${m.id}/${m.slug}`);
